...
int dist_measure (population_real *mixed_pop, individual_real *a, individual_real *b, double sigma)
{
    ...
    for (i = 0; i < number_objective; i++)
    {
        distance1 += weights_obj[i] * pow ((a->obj[i] - reference_point[i]) / (nadir_point[i] - ideal_point[i]), 2);
        distance2 += weights_obj[i] * pow ((b->obj[i] - reference_point[i]) / (nadir_point[i] - ideal_point[i]), 2);
    }
