...
void fill_r_nondominated_sort (population_real *new_pop, population_real *mixed_pop, double sigma)
{
    ...
    temp1 = pool;
    double distance1,distance2, distance_measure;
    double *distance_array;
    double dist_min, dist_max, temp_dist;
        ...
        do
        {
            ...
            do
            {
                ...
                if (flag == 0)
                {
                    ...
                    {
                        distance1 += weights_obj[m] * pow ((mixed_pop->ind[temp1->index].obj[m] - reference_point[m]) / (max_obj[m] - min_obj[m]), 2);
                        distance2 += weights_obj[m] * pow ((mixed_pop->ind[temp2->index].obj[m] - reference_point[m]) / (max_obj[m] - min_obj[m]), 2);
                    }
