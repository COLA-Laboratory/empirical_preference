...
int compare(double *temp_dist,  int front_size);
...
/* Environmental selection in EMO with using RNSGA-II as a general-purpose optimiser */
void fill_Rgeneral_nondominated_sort(population_real *new_pop, population_real *mixed_pop, double* reference_point, double* weights, double epsilon)
{
    int i, j;
    int flag, end;
    int front_size = 0;
    ...
    do
    {
        ...
        {
            r_general_crowding_fill (mixed_pop, new_pop, i, front_size, elite, reference_point, weights, epsilon);
            archieve_size = popsize;
            for (j = i; j < popsize; j++)
                new_pop->ind[j].rank = rank;
        }
...
/* Similar to the normal crowding distance assignment procedure in NSGA-II, but considering the preference information
 * by taking the distance to the reference point in calculation with using RNSGA-II as a general-Purpose optimiser */
void r_general_crowding_fill (population_real *mixed_pop, population_real *new_pop, int count, int front_size, list *elite, double* reference_point, double* weights, double epsilon)
{
    int i, j, k;
    int temp_idx,remaining_cnt,temp_index;
    int *front_index;
    double temp;
    double *temp_dist;
    ...
    max_obj           = (double *) malloc (number_objective * sizeof(double));
    min_obj           = (double *) malloc (number_objective * sizeof(double));
    temp_dist         = (double *) malloc (front_size * sizeof(double));
...
// Find the maximum and minimum values of each dimension.
    ...
    for(i=0;i <number_weight;i++)
    {
        ...
        for (j = 0; j < front_size; j++)
        {
            ...
            {
                temp_dist[j] += weights[j] * pow((ind.obj[k] - lambda[i][k]) / (max_obj[k] - min_obj[k]), 2);
                //temp_obj=lambda[i][k] * min_obj[k];
               // temp_dist[j] += weights[k] * pow((ind.obj[k] -temp_obj), 2);
            }

        }
        for (j = 0; j < front_size; j++)
        {
            temp_index = compare(temp_dist,front_size);
            if(rank[temp_index] > (j+1) )
                rank[temp_index] = (j+1);
        }
    ...
    for (i = 0; i < front_size; i++)
    {
        mixed_pop->ind[get_item (elite, i)->index].fitness = front_size - rank[i];
    }
