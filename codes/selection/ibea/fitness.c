...
void pbeaGeneral_calcFitness (population_real *mixed_pop, double **Indicator_value, int size, double* weights, double** reference_point, double specificity)
{
    ...
    double *asf_values;//[size];
    population_real* pop;
    asf_values           = (double *) malloc (size * sizeof(double));
    pop = mixed_pop;
    ...
    for (i = 0; i < size; i++)
    {
        asf_values[i] = asf_pbeaGeneral (pop->ind[i].obj, reference_point, weights);
...
double asf_pbeaGeneral (double *a, double** reference_point, double *weights)
{
    int i,j,index=0;
    double sum, ASF_value;
    double temp, weights_s, ASF_max,ASF_min;
    double *ASF_tensor;
    sum     = 0;
    ASF_tensor           = (double *) malloc (number_weight * sizeof(double));
    for (j=0;j < number_weight;j++)
    {
        ASF_max = -INF;
        for (i = 0; i < number_objective; i++)
        {
            temp = a[i] - reference_point[j][i];
            sum += temp;
            weights_s = weights[i] * temp;
            if (ASF_max < weights_s)
                ASF_max = weights_s;
        }
        ASF_tensor[j] = ASF_max;
    }
    ASF_min = INF;
    for (j=0;j < number_weight;j++)
    {
        if(ASF_min > ASF_tensor[j])
        {
            ASF_min = ASF_tensor[j];
            index = j;
        }
    }
    for (i = 0; i < number_objective; i++)
    {
        temp = a[i] - reference_point[index][i];
        sum += temp;

    }

    ASF_value = ASF_min + 0.00001 * sum;
    free(ASF_tensor);
    return ASF_value;
}
